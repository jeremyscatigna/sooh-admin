...
const doIHaveSignaledThisPost = (item, user) => {
  if (item.signaling) {
    return (
      item.signaling.filter((signaling) => signaling.userId === user.uid)
        .length > 0
    );
  }
  return false;
...
function FeedPost({ item, handleBlockUser }) {
  ...
  const [like, setLike] = React.useState(
    item.likes ? doILikeThisPost(item, user) : false
  );
  const [signaled, setSignaled] = React.useState(
    item.signaling ? doIHaveSignaledThisPost(item, user) : false
  );
  ...
  const handleUserSelect = (firstName, lastName) => {
    ...
    setComment(
      `${comment.substring(0, lastAtIndex)}@${firstName} ${lastName} `
    );
    setIsUserListVisible(false);
  };
  ...
  const handleSignalingContent = async (e, post) => {
    ...
    if (post.signaling) {
      if (signaled) {
        updatedSignaling = post.signaling.filter(
          (signaling) => signaling.userId !== user.uid
        );
    ...
    sendEmail(
      ...
      `L'utilisateur ${user.firstName} ${
        user.lastName
      } a signalé le contenu suivant : ${post.text} poster par ${
        post.userFirstName
      } ${post.userLastName}} (${
    ...
    <div key={item.uid} className="bg-card shadow-md rounded-xl p-5">
        ...
        >
          <li>
            <div
              ...
              className="font-medium text-sm text-primary hover:text-pink-500 cursor-pointer flex py-1 px-3"
              to="#0"
            >
              Bloquer
            </div>
      ...
      <footer className="flex items-center space-x-4">
        ...
        >
          ...
          />
          <div
            className={`text-sm ${like ? "text-pink-500" : "text-secondary"}`}
          >
              ...
              <li className="p-3 bg-hover rounded-xl" key={comment.uid}>
                <div className="flex items-center space-x-3">
                  {comment.userAvatar ? (
                    ...
                    <Avvvatars
                      value={`${comment.userFirstName} ${comment.userLastName}`}
                    />
                  )}
                  <div>
                    <div className="text-xs text-white">
                      ...
                      <span className="text-secondary">
                        Il y a {dayjs(comment.timestamp).fromNow(true)}
                      </span>
                    </div>
      ...
      <div className="flex flex-col items-center w-full space-x-3 mt-3">
        <div className="flex items-center space-x-3 w-full mt-3">
          {user.avatar ? (
            <img
              className="rounded-full shrink-0 w-8 h-8 object-fit"
              src={user.avatar}
              width="32"
              height="32"
              alt="User 02"
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      {filteredUser.avatar ? (
                        ...
                        <Avvvatars
                          value={`${filteredUser.firstName} ${filteredUser.lastName}`}
                        />
                      )}
                      <div>
                        <div className="text-xs text-white">
